...
class HausaStemmer:
    ...
    def __step_1b(self):
        ...
        """
        #IMPROVED to stem rigimamme, hadamamme, sokakke, 
        starts = len(self.b)-3
        ends = len(self.b)-2
        
        if len(self.b) >=6:            
            if self.__cons(starts) and self.__cons(ends):
                r =  self.b[starts:]
                if self.__ends_with(r):
                    self.__r("")
                
        #The only instructions before improvement        
        elif self.__starts_with("mai") and len(self.b) > 5:
            self.k0 = 3
    ...
    def __step_2(self):
        ...
        #IMPROVED to acknowledge Standard Hausa rubuce-rubuce, karance-karance 
        if self.__ends_with("che") or self.__ends_with("ce"):
            self.__r("tu")
